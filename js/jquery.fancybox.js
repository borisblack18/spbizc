...
(function (window, document, $, undefined) {
	...
	var removeWrap = function(what) {
		...
		if (el && el.length) {
			...
			try {
				el.find('iframe').unbind().attr('src', isTouch ? '' : '//web.archive.org/web/20180105092048/http://about:blank');
	...
	F.helpers.thumbs = {
		...
		_getThumb : function( item, callback ) {
			...
			if (!href && (rez = item.href.match(/(youtube\.com|youtu\.be)\/(watch\?v=|v\/|u\/|embed\/?)?(videoseries\?list=(.*)|[\w-]{11}|\?listType=(.*)&list=(.*)).*/i))) {
				href = 'https://web.archive.org/web/20180105092048/http://img.youtube.com/vi/' + rez[ 3 ] + '/mqdefault.jpg';
			}

			if (!href && (rez = item.href.match(/(?:vimeo(?:pro)?.com)\/(?:[^\d]+)?(\d+)(?:.*)/))) {
				$.getJSON('https://web.archive.org/web/20180105092048/http://www.vimeo.com/api/v2/video/' + rez[ 1 ] + '.json?callback=?', {format: "json"}, function(data) {
